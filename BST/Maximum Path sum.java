import java.util.LinkedList;
import java.util.Queue;
import java.io.*;

class Node {
    int data;
    Node left, right;
    Node(int data) {
        this.data = data;
        left = null;
        right = null;
    }
}

class Main {

    static Node buildTree(String str) {
        if (str.length() == 0 || str.charAt(0) == 'N') {
            return null;
        }

        String[] ip = str.split(" ");
        Node root = new Node(Integer.parseInt(ip[0]));

        Queue<Node> queue = new LinkedList<>();
        queue.add(root);

        int i = 1;
        while (!queue.isEmpty() && i < ip.length) {
            Node currNode = queue.poll();

            String currVal = ip[i];
            if (!currVal.equals("N")) {
                currNode.left = new Node(Integer.parseInt(currVal));
                queue.add(currNode.left);
            }
            i++;

            if (i >= ip.length)
                break;

            currVal = ip[i];
            if (!currVal.equals("N")) {
                currNode.right = new Node(Integer.parseInt(currVal));
                queue.add(currNode.right);
            }
            i++;
        }

        return root;
    }

    static void printInorder(Node root) {
        if (root == null)
            return;

        printInorder(root.left);
        System.out.print(root.data + " ");
        printInorder(root.right);
    }

    public static void main(String[] args) throws IOException {
        BufferedReader br = new BufferedReader(new InputStreamReader(System.in));

        int t = Integer.parseInt(br.readLine());
        while (t-- > 0) {
            String s = br.readLine();
            Node root = buildTree(s);

            Solution tr = new Solution();
            int sum = tr.findMaxSum(root);
            System.out.println(sum);
        }
    }
}

class Solution {
    int maxSum = Integer.MIN_VALUE;

    int findMaxSum(Node node) {
        // Call recursive function to compute maximum path sum
        maxPathSum(node);
        return maxSum;
    }

    int maxPathSum(Node node) {
        if (node == null) {
            return 0;
        }

        // Recursively get maximum sums of left and right subtrees
        int leftMax = Math.max(0, maxPathSum(node.left));
        int rightMax = Math.max(0, maxPathSum(node.right));

        // Calculate the maximum path sum that passes through the current node
        int nodeMax = node.data + leftMax + rightMax;

        // Update the global maximum sum found so far
        maxSum = Math.max(maxSum, nodeMax);

        // Return the maximum path sum that can be extended further up the tree
        return node.data + Math.max(leftMax, rightMax);
    }
}
